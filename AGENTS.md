...
## Installer + versioning (landed 2026-08-27, `2cb71ea`)

- NSIS 3.12 at `C:\Program Files (x86)\NSIS`. Script `packaging/installer.nsi`;
  assets generated by `imaginer-ui/examples/make-installer-assets.rs` from the
  SVG sources: `imaginer.ico` (own ICO encoder, 512px source), `header.bmp`
  (150x57 exact, NSIS requires it) and `sidebar.bmp` (164x314 exact) on the
  app's own palette. Rebuild: release build → run the example → makensis
  (paths in the nsi are relative to `packaging/`, exe from `target/release`).
- Output `dist/Imaginer-<ver>-setup.exe` (~3.2MB before jxl-oxide, 3.7 after;
  dist/ and generated ico/bmp stay untracked). Tested: silent `/S` install to
  `Program Files\Imaginer` works, ARP entry lands in WOW6432Node (NSIS is
  32-bit — normal, Settings shows it), shortcuts + uninstaller verified, and
  the installed exe opened the JXL sample.
- Installer kills a running `imaginer.exe` before copying (taskkill), so an
...
- File-extension badge on Explorer thumbnails (Illustrator-style, bottom-right)
  was evaluated 2026-08-27 and **rejected by the user as not worth it**. The
  Ai badge is baked into Adobe's .ico art, not a shell feature; doing it for
  all formats means a COM thumbnail provider (explorer.exe crash risk) — only
  the baked-ico trick is cheap, and it covers just formats without system
  thumbnails (jxl/psd/svg). Do not re-open without the user asking.

## JXL notes (landed 2026-08-27, `2cb71ea`)

- `jxl-oxide` 0.12 API, learned the hard way: the useful path is
  `JxlImage::builder().read(r)` → `render_frame(0)` → `Render::stream()` →
  `ImageStream::write_to_buffer::<u8>`. `write_to_buffer` lives on
  **ImageStream**, not FrameBuffer; `stream()` applies orientation (so the
  Decoded carries `Orientation::Normal` — applying the EXIF one would flip
  twice); its width/height/channels are all `u32`. The crate's `Result` error
  is `Box<dyn Error + Send + Sync>`, hence `DecodeError::Jxl` stores a boxed
  source, not a concrete type.
- Still-only (first keyframe); animated JXL plays nothing yet. Decode-only —
  no export, and the two shell scripts list `.jxl` (association + convert menu)
  in the same commit, as their comments require.
- Tests: `#[ignore]`d `decodes_the_jxl_sample` drives the real file in
  Downloads (0.3s decode); a dispatch test pins broken-JXL → `DecodeError::Jxl`
  rather than the SVG fallback.
- Trap: `cargo test --release` with the workspace's fat-LTO profile takes
  >5min to compile; use debug tests for iteration, release only for the
  startup benchmarks.
